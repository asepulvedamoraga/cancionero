...
<section class="card mb-4" id="song-selector" data-song-selector data-search-url="{{ route('repertoires.edit',$repertoire) }}">
    ...
    <div class="card-body border-bottom">
        <form method="GET" action="{{ route('repertoires.edit',$repertoire) }}" role="search" data-selector-search-form>
            <div class="row g-2 align-items-end">
                ...
                <div class="col-lg-1 d-grid"><button class="btn btn-outline-secondary" type="button" data-clear-search><i class="bi bi-x-circle"></i></button></div>
            </div>
        </form>
    </div>

    <form method="POST" action="{{ route('repertoires.songs.store',$repertoire) }}" data-selector-submit-form>
        @csrf
        <div class="card-body">
            <div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-3">
                <span class="small fw-semibold" data-selected-count>0 canciones seleccionadas</span>
            </div>
            <div class="song-selector-picked mb-3" data-selected-songs-panel>
                <p class="small text-secondary mb-2">Canciones a agregar</p>
                <div class="song-selector-picked__chips" data-selected-songs-chips>
                    <span class="song-selector-picked__empty">Aún no has seleccionado canciones.</span>
                </div>
            </div>

            <div data-selector-hidden-inputs></div>

            <div class="song-selector-results" data-selector-results>
                @include('repertoires.partials.song-selector-results', ['songs' => $songs])
            </div>
        </div>
        ...
        <div class="repertoire-song-list" data-repertoire-songs data-reorder-url="{{ route('repertoires.songs.reorder',$repertoire) }}">
            @forelse($repertoire->songs as $song)
                <article class="repertoire-song-item" data-song-id="{{ $song->id }}">
                    ...
                    <div class="flex-grow-1"><div class="d-flex flex-wrap justify-content-between gap-2"><strong>{{ $song->title }}</strong><span class="badge text-bg-light">{{ $song->page_count }} {{ $song->page_count === 1 ? 'página' : 'páginas' }}</span></div><div class="small text-secondary">{{ $song->author ?: 'Autor no indicado' }}</div><div class="small text-secondary"><i class="bi bi-person"></i> Subida por {{ $song->owner->name }}</div>@php($selectedToneId = $song->pivot->song_tone_id ?: optional($song->tones->firstWhere('is_default', true))->id)@php($selectedTone = $song->tones->firstWhere('id', $selectedToneId))<div class="small text-secondary">Tonalidad: {{ $selectedTone?->name ?: 'Original' }}</div>@if($song->page_count===0)<div class="small text-warning"><i class="bi bi-exclamation-triangle"></i> Sin páginas disponibles</div>@endif<form class="mt-2" method="POST" action="{{ route('repertoires.songs.update',[$repertoire,$song]) }}">@csrf @method('PUT')<div class="row g-2"><div class="col-md-4"><select class="form-select form-select-sm" name="song_tone_id">@foreach($song->tones as $tone)<option value="{{ $tone->id }}" @selected($tone->id === $selectedToneId)>{{ $tone->name }}</option>@endforeach</select></div><div class="col-md-8"><div class="input-group input-group-sm"><input class="form-control" name="notes" value="{{ $song->pivot->notes }}" maxlength="2000" placeholder="Nota particular para esta canción"><button class="btn btn-outline-secondary">Guardar</button></div></div></div></form></div>
