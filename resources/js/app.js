...
document.addEventListener('DOMContentLoaded',()=>{ initFilePreview(); initCopyButtons(); initSongFileSorter(); initSongDuplicateCheck(); initRepertoireSorter(); initRepertoireSelector(); initSongUploadFeedback(); document.querySelectorAll('form[data-confirm]').forEach(form=>form.addEventListener('submit',event=>{ if(!window.confirm(form.dataset.confirm)){ event.preventDefault(); return; } const expected=form.dataset.confirmText?.trim(); if(!expected)return; const answer=window.prompt(form.dataset.confirmPrompt || `Escribe ${expected} para confirmar.`,''); if(answer?.trim()!==expected)event.preventDefault(); })); });
